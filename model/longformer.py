...
class MyConfig(object):
    def __init__(self, task):
        ...
        self.save_path = "evaluation/longformer.ckpt"  # 模型结果
...
class MyModel(nn.Module):
    def __init__(self, config):
        ...
        self.longformer = AutoModel.from_pretrained(config.model_path)
    ...
    def forward(self, batch):
        ...
        with torch.no_grad():
            # longformer模型提取语义特征
            x1 = self.longformer(encoding1['input_ids'], attention_mask=encoding1['attention_mask']).pooler_output
            x2 = self.longformer(encoding2['input_ids'], attention_mask=encoding2['attention_mask']).pooler_output
